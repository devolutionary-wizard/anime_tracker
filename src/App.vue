...
const my_anime = ref<any>([])
const search_results =ref<any>([])

const my_anime_asc = computed(()=>{
  return my_anime.value.sort((a:any,b:any)=>{
...
const searchAnime = ()=>{
  ...
  fetch(url).then(res=>res.json()).then(res=>search_results.value = res.data)
}
...
const increaseWatch = (anime :any) => {
	anime.watched_episodes++
	localStorage.setItem('my_anime', JSON.stringify(my_anime.value))
}
const decreaseWatch = (anime :any) => {
	anime.watched_episodes--
	localStorage.setItem('my_anime', JSON.stringify(my_anime.value))
}
